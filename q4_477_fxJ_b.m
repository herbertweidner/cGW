...
if 1==1000000000000000000000000000, erg=zeros(65,2); 
...
end, plot(erg(:,2)) %Optimum bei k3=22.6
end

y0=y.*sin(t*fD+2.26); %++++++++++++++++++++++++++++

fGW=fGW0-3.9e-6; %Offset
...
h4=window_sinc_filter(2e5,0.5e-9,1,1/Ts,'low','blackman');
...
%{
ax=0.2; px=3; fx=38.9735e-9; D1=100.85; % 
...
%}
ax=4.21563; px=1.78884; fx=38.70143e-9; D1=45.16;
ax2=4.5932; px2=0.9302; fx2=14.280667e-9; % C
ax3=1.83024; px3=4.06893; fx3=9.34162e-10; %Frequenzzähler M
ax4=0; px4=3; fx4=4.29075e-10; % Frequenzzähler L 
ax5=3.617435; px5=3.86016; fx5=94.9618e-9; % D
ax6=2.99207; px6=5.44064; fx6=3.41998e-9; % F
ax7=0; px7=3; fx7=3.5202e-10; % Frequenzzähler  Jahre
ax8=0; px8=0.3; fx8=4e-9; % 
ax9=0; px9=3; fx9=2.62448e-9; % E
ax10=0; px10=3; fx10=6.12776e-9; % Jahre
ax11=0; px11=3; fx11=0.98e-9; % H
ax12=0; px12=3; fx12=4.8e-9; % J
...
ax14=0; px14=3; fx14=1.39276e-10; % Frequenzzähler
ax15=0; px15=3; fx15=1.20464e-10; % Frequenzzähler
axJ=4.2417; pxJ=4.8372; fxJ=31.637671e-9; %31.69 nHz
...
da=1e-3; da2=1e-2; da3=1e-5; da4=1e-5; da5=1e-3; da6=1e-3; da7=1e-5; da8=1e-3; daJ=1e-4; da9=1e-3; da10=1e-2; da11=1e-2; da12=1e-3; da13=1e-3; da14=1e-5; da15=1e-5; dD=1;

R=zeros(595,52); erg=zeros(3,2); Limi=-11; ww=-3.30806e-05; k3alt=1;
for k3=1:595
...
for k=1:2, k1=D1+(k-2)*dD; %D1 bestimmen
...
D1=D1-erg(2,2)*dD/(erg(2,2)-erg(1,2)); %genauer 0-Durchgang 
%D1=min(350,max(D1,-150));
...
if abs(k)<1
    if dpJ<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, dpJ=dpJ/2;
    end
end
end
...
if abs(k)<1
    if daJ<1e-6 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, daJ=daJ/2;
    end
end
...
if abs(k)<1
    if df9<2e-15 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, df9=df9/2;
    end
end
end
...
if abs(k)<1
    if dp2<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, dp2=dp2/2;
    end
end
end
...
if abs(k)<1
    if df2<2e-15 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, df2=df2/2;
    end
end
end
...
if abs(k)<1
    if da2<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, da2=da2/2;
    end
end
...
if abs(k)<1
    if dp3<1e-7 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, dp3=dp3/2;
    end
end
end
...
if abs(k)<1
    if df3<1e-17 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, df3=df3/2;
    end
end
end
...
if abs(k)<1
    if da3<1e-7 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, da3=da3/2;
    end
end
...
if abs(k)<1
    if dp4<1e-7 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, dp4=dp4/2;
    end
end
end
...
if abs(k)<1
    if df4<1e-18 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, df4=df4/2;
    end
end
end
...
if abs(k)<1
    if da4<1e-7 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, da4=da4/2;
    end
end
...
if abs(k)<1
    if dp5<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, dp5=dp5/2;
    end
end
end
...
if abs(k)<1
    if df5<2e-15 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, df5=df5/2;
    end
end
end
...
if abs(k)<1
    if da5<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, da5=da5/2;
    end
end
...
if abs(k)<1
    if dp6<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, dp6=dp6/2;
    end
end
end
...
if abs(k)<1
    if df6<2e-15 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, df6=df6/2;
    end
end
end
...
if abs(k)<1
    if da6<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, da6=da6/2;
    end
end
...
if abs(k)<1
    if dp7<1e-7 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, dp7=dp7/2;
    end
end
end
...
if abs(k)<1
    if df7<2e-17 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, df7=df7/2;
    end
end
end
...
if abs(k)<1
    if da7<1e-7 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, da7=da7/2;
    end
end
...
if abs(k)<1
    if dp8<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, dp8=dp8/2;
    end
end
end
...
if abs(k)<1
    if df8<2e-15 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, df8=df8/2;
    end
end
end
...
if abs(k)<1
    if da8<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, da8=da8/2;
    end
end
...
if abs(k)<1
    if dp9<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, dp9=dp9/2;
    end
end
end
...
if abs(k)<1
    if da9<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, da9=da9/2;
    end
end
...
if abs(k)<1
    if dp10<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, dp10=dp10/2;
    end
end
end
...
if abs(k)<1
    if df10<2e-15 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, df10=df10/2;
    end
end
end
...
if abs(k)<1
    if da10<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, da10=da10/2;
    end
end
...
if abs(k)<1
    if dp11<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, dp11=dp11/2;
    end
end
end
...
if abs(k)<1
    if df11<2e-15 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, df11=df11/2;
    end
end
end
...
if abs(k)<1
    if da11<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, da11=da11/2;
    end
end
...
if abs(k)<1
    if dp12<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, dp12=dp12/2;
    end
end
end
...
if abs(k)<1
    if df12<2e-15 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, df12=df12/2;
    end
end
end
...
if abs(k)<1
    if da12<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, da12=da12/2;
    end
end
...
if abs(k)<1
    if dp13<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, dp13=dp13/2;
    end
end
end
...
if abs(k)<1
    if df13<2e-15 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, df13=df13/2;
    end
end
end
...
if abs(k)<1
    if da13<1e-5 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, da13=da13/2;
    end
end
...
if abs(k)<1
    if dp14<1e-7 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, dp14=dp14/2;
    end
end
end
...
if abs(k)<1
    if df14<2e-17 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, df14=df14/2;
    end
end
end
...
if abs(k)<1
    if da14<1e-7 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, da14=da14/2;
    end
end
...
if abs(k)<1
    if dp15<1e-7 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, dp15=dp15/2;
    end
end
end
...
if abs(k)<1
    if df15<2e-17 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, df15=df15/2;
    end
end
end
...
if abs(k)<1
    if da15<1e-7 || max(erg(:,2))<1.4*min(erg(:,2)), break
    else, da15=da15/2;
    end
end
...
if k3>k3alt+10 && mean(R(k3-5:k3,15))<Limi %ruhige Iteration? 
    ...
    ww=ww-mean(R(max(10,k3-30):k3,31))/20; disp(ww)
...
%PSD berechnen
h6=window_sinc_filter(5e3,100e-9,1,1/Ts,'low','blackman');
y1=conv(ys,h6,'same').*stt+conv(yc,h6,'same').*ctt;
yi=conv(ys,h6,'same').*stt-conv(yc,h6,'same').*ctt; %invers
[sp,f]=zeig_sp2(y1,Ts,2000.4); [spi,f]=zeig_sp2(yi,Ts,2000.4);
plot(f,sp,f,spi)
k=16*1024;[sp,f]= pwelch(cat(1,y1,zeros(3e5,1)),rectwin(k),k/8,16*k,1/Ts);plot(1e9*f,sp)
