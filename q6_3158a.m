...
fGW0=6.315173E-6; %HW95=fGW0*360*3600; optimiert

tD=23.93447192; fD=1/(tD*3600); 
tJ=365.25636042; fJ=1/tJ/24/3600; 
[~,~]=zeig_sp2(y,Ts,-200000.3);
y=decimate(y,10); Ts=10*Ts; [sp,f]=zeig_sp2(y,Ts,200000.3);
k=1;while f(k)<fGW0,k=k+1;end,k=k-50:k+50; plot(1e6*f(k),sp(k))
t=(1:Ts:21/fJ)'; t=2*pi*t(1:length(y));

y0=y.*(1.5+sin(t*fD+1.9)); %++++++++++++++++++++++++++++

Offset=5.65e-6; fGW=fGW0-Offset; korr=6e-11; %zentrieren
y= filtTrans_n_Hz(y0,Ts,2e3,fGW0,fGW,0.6e-6); %bis 2*600nHz wegen Mod-Index
y=decimate(y,10); Ts=10*Ts; zeig_sp2(y,-Ts,'log'); 
[sp,f]=zeig_sp2(y,Ts,-300000.3);k=1;while f(k)<fGW,k=k+1;end,q1=k-33:k+44;
plot(1e9*f(q1),sp(q1)), title('IF = verschobene fGW'),xlabel('Frequenz (nHz)')
t=(1:Ts:21/fJ)'; tt=1:(length(y)); fZF=1/Ts/10; %~1e-6;
    dftt=1e-20*t(tt); t=2*pi*t(tt); %21 Jahre FM darstellen
    tt=(tt)'*2*pi*fZF*Ts; %fixe ZF wegen variablem Oszillator
    stt=sin(tt); ctt=cos(tt);
j=0.6e-9; h1=window_sinc_filter(1e5,fZF-j,fZF+j,1/Ts,'bandpass','Blackman');
h4=window_sinc_filter(2e5,0.6e-9,1,1/Ts,'low','blackman');
h5=window_sinc_filter(2e4,10e-9,1,1/Ts,'low','blackman');
%j=5*0.08e-9;
...
%}
ax=6.77; px=1.82; fx=38.585e-9; drift=1256; 
ax2=6.435; px2=0.4; fx2=14.256e-9; % C
ax3=1.97; px3=2.95; fx3=10.986e-10; %Frequenzzähler M
ax4=1.62; px4=6.24; fx4=8.0e-10; % Frequenzzähler L 
ax5=2.03; px5=2.84; fx5=95.43e-9; % D
ax6=6.5; px6=5.74; fx6=3.134e-9; % F
ax7=1.152; px7=3.935; fx7=5.75e-10; % Frequenzzähler  Jahre
ax8=1.94; px8=-0.12; fx8=74.7e-9; % 
ax9=0; px9=0; fx9=63e-9; % 
ax10=1.6; px10=3.38; fx10=33.12e-9; % Jahre
ax11=0; px11=3; fx11=0.98e-9; % H
ax12=0; px12=3; fx12=4.8e-9; % J
ax13=0; px13=3; fx13=1.24e-9; % K
ax14=0; px14=3; fx14=1.39276e-10; % Frequenzzähler
ax15=0; px15=3; fx15=1.20464e-10; % Frequenzzähler
axJ=7.3; pxJ=4.69; fxJ=31.545e-9; %31.69 nHz

dD=1; k=695; %k=Anzahl der Iterationen; 500<k<2e4
R=zeros(k,52); erg=zeros(3,2); Limi=-11; k3alt=1; j=0;
for k3=1:size(R,1)
...
[ax10,fx10,px10,z2]=ITER(ax10,fx10,px10,t,fGW,drift,y,z2,h4,dftt,stt,ctt);
...
%Spiegelspektrum zeigen -> übereinstimmende Linien?
...
plot(1e9*f,sp/1e5), xlabel('IF-frequency (nHz)')
[k,j]=max(sp); k1=mean(sp(1:j-2e3)); k3=mean(sp(j+2e3:end));
Limi=2*k/(k1+k3);
...
%%%%%%%%%%
[pxx,f] = pwelch(y1,500,400,1000,1/Ts);
...
if A==0, return, end 
dp=0.1; df=1e-12; da=1e-3; erg=zeros(3,2); Ts=360000;

z2=z2-A*sin(P+t*F); %nach 3 Iterationen wieder einsetzen
for j=1:123
for k=1:3, pxx=P+dp*(k-2);
    z=A*sin(pxx+t*F)+z2; 
    yi=t.*(fGW+drift*dftt)+z; 
    ys=y.*sin(yi); yc=y.*cos(yi); 
